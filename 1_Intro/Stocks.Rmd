...
```{r}
df_IBM_monthly <- dfos$IBM %>% group_by(Date) %>% summarise(mean(StockPrice))
df_IBM_monthly
colnames(df_IBM_monthly)<-c('1','2')
df_IBM_monthly$name<- month.abb[df_IBM_monthly$month]

df_IBM_monthly
```

```{r}
ggplot(df_IBM_monthly,aes(x=month,y=price,group=1))+ geom_line()+ scale_x_continuous(breaks=1:12,labels=df_IBM_monthly$name)

```
```{r}
df_group_month <- df.all.gathered %>% group_by(month(Date),company) %>% summarise(mean(price))
colnames(df_group_month)<-c('month','company','price')


df_group_month

ggplot(df_group_month,aes(x=month,y=price,group=company,colour=company))+ geom_line()+
scale_x_continuous(breaks=1:12,labels=month.abb[c(1:12)])
```
